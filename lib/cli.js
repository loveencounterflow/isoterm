(function() {
  ...
  (async() => {    //###########################################################################################################
    var PU, Randex, _, i, port, randex;
    // await run()
    PU = require('port-used');
    Randex = require('randexp');
    randex = new Randex(/^5[0-9]{2}11$/);
    randex = new Randex(/^[1-9]111$/);
    for (_ = i = 1; i <= 10; _ = ++i) {
      port = randex.gen();
      if (!/^[0-9]+$/.test(port)) {
        throw new Error(`the supplied pattern resulted in an illegal port number: ${rpr(port)}`);
      }
      port = parseInt(port, 10);
      if (!((1024 < port && port < 65535))) {
        whisper(`port number not in range: ${rpr(port)}`);
        continue;
      }
      urge('^3344^', port, (await PU.check(port)));
    }
    return null;
  })();
